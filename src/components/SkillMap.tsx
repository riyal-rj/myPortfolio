...
import {
  Code, Database, Server, GitBranch, Brain, MonitorSmartphone,
  MessageSquare, FileCheck, GitMerge, Workflow,
} from "lucide-react";
...
import {
  ...
  SiNextdotjs, SiFastapi, SiExpress, SiSalesforce
} from 'react-icons/si';
import { VscAdd } from "react-icons/vsc";

// --- ENHANCED SKILL NODE WITH ICON FALLBACK ---
const SkillNode = ({ data }) => {
  const [isExpanded, setIsExpanded] = useState(false);
  const [imgError, setImgError] = useState(false); // New state to handle missing images
  
  const IconComponent = data.iconComponent || Code;
  ...
  return (
    <div className="skill-node bg-card border border-border rounded-lg p-3 shadow-md min-w-[120px]">
      ...
      <div className="flex flex-col items-center cursor-pointer" onClick={() => setIsExpanded(!isExpanded)}>
        {/* If iconUrl exists and hasn't failed, show img. Otherwise, show IconComponent */}
        {data.iconUrl && !imgError ? (
          <img 
            src={data.iconUrl} 
            alt={data.label} 
            className="w-6 h-6 mb-2 animate-bounce" 
            onError={() => setImgError(true)} // Switches to IconComponent if file is missing
          />
        ) : (
...
const SkillMap = () => {
  const initialNodes = [
    ...
    // Enterprise & CRM (Salesforce Stack)
    { id: 'enterprise', type: 'skillNode', position: { x: 1500, y: 50 }, data: { label: 'Enterprise Systems', iconComponent: SiSalesforce, description: 'Managing enterprise-level CRM and Contract Lifecycle Management.' } },
    { id: 'salesforce', type: 'skillNode', position: { x: 1500, y: 170 }, data: { label: 'Salesforce', iconUrl: '/logos/salesforce.jpg', iconComponent: SiSalesforce, level: 'Advanced', description: 'World #1 CRM platform for sales, service, and marketing.' } },
    { id: 'apex', type: 'skillNode', position: { x: 1500, y: 290 }, data: { label: 'Apex', iconUrl: "/logos/apex-sf.png", iconComponent: Code, level: 'Expert', description: 'Strongly typed, object-oriented programming language for Salesforce.' } },
    { id: 'lwc', type: 'skillNode', position: { x: 1500, y: 410 }, data: { label: 'LWC', iconUrl: "/logos/lwc-sf.png", iconComponent: DiReact, level: 'Advanced', description: 'Lightning Web Components: Modern custom elements for Salesforce.' } },
    { id: 'flows', type: 'skillNode', position: { x: 1500, y: 530 }, data: { label: 'Flows & Triggers', iconUrl: "/logos/flows.png", iconComponent: Workflow, level: 'Expert', description: 'Declarative and programmatic automation within Salesforce and Docusign.' } },
    { id: 'csharp', type: 'skillNode', position: { x: 1500, y: 650 }, data: { label: 'C# Expressions', iconUrl: "/logos/csharp.png", iconComponent: TbBrandCSharp, level: 'Expert', description: 'Customizing workflows and complex logic via C# expressions.' } },

    // CLM & DocuSign
    { id: 'clm-section', type: 'skillNode', position: { x: 1800, y: 50 }, data: { label: 'CLM & Digital Sign', iconComponent: FileCheck, description: 'Automating contract lifecycles and digital signatures.' } },
    { id: 'docusign', type: 'skillNode', position: { x: 1800, y: 200 }, data: { label: 'DocuSign', iconUrl: "/logos/docusign.svg", iconComponent: FileCheck, level: 'Intermediate', description: 'Electronic signature and agreement cloud integration.' } },
    { id: 'clm-workflow', type: 'skillNode', position: { x: 1800, y: 350 }, data: { label: 'CLM Workflow', iconUrl: "/logos/workflow.png", iconComponent: GitMerge, level: 'Expert', description: 'Architecting end-to-end DocuSign CLM workflows using C# expressions for advanced document generation, routing, and global business flow optimization.' } },
  ];
  ...
  const initialEdges = [
    ...
    // Enterprise & CLM Connections
    { id: 'e-source-enterprise', source: 'source-code', target: 'enterprise', animated: true },
    { id: 'e-enterprise-clm', source: 'enterprise', target: 'clm-section', animated: true },
    { id: 'e-sf-apex', source: 'salesforce', target: 'apex' },
    { id: 'e-sf-lwc', source: 'salesforce', target: 'lwc' },
    { id: 'e-sf-flows', source: 'salesforce', target: 'flows' },
    { id: 'e-sf-csharp', source: 'salesforce', target: 'csharp' },
    { id: 'e-clm-ds', source: 'clm-section', target: 'docusign' },
    { id: 'e-clm-wf', source: 'clm-section', target: 'clm-workflow' },
    { id: 'e-csharp-wf', source: 'csharp', target: 'clm-workflow', animated: true },
    { id: 'e-clm-deploy', source: 'clm-section', target: 'deployment', animated: true },

    // DevOps & DB connections
